...
class OrbitSlider extends CWidget {
    ...
    public function makeImages() {
        $folder = "upload/news/";  // folder for Picture
        $html = '<div id="' . $this->name . '">' . "\n";
        $html .= '<ul id="' . $this->name . 'Content">' . "\n";
        $i = 0;
        $caption = '';
        //foreach ($this->images as $img) {
        foreach ($this->model as $rw) {
            
            $html .= '<li class="' . $this->name . 'Image">' . "\n";

            // บรรทัดนี้ original ไม่มี width & height
            $image = '<img src="' .  $folder . $rw->id . "/" . $rw->news_pic . '" width="' . $this->mywidth . '" height="'. $this->myheight .'" ';
            //if (isset($img['caption'])) {
                $image.=' rel="img' . $i . '"';
                $caption .= '<span class="orbit-caption" id="img' . $i . '">' . $rw->news_head . '</span>' . "\n";
            //}
            $i++;
            $image.= ' />' . "\n";
            ...
            //if (isset($img['url'])) {
                $d = $image;
                $image = '';
                $image.='<a href="' . Yii::app()->createUrl('/Home/ReadNews&id=' . $rw->id) . '">' . $d . '</a>';
            //}
            $html.=$image;
            $html .= '</li>' . "\n";
        }
