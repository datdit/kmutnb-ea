<script type="text/javascript">
    ...
    if (confirm('ยืนยันการลบไฟล์')) {
            $.ajax({
                data: {id: id},
                url: "<?= Yii::app()->params['MyBaseURL']; ?>Gallery/PicDel&id=" + id,
                type: 'POST',
                dataType: 'json',
                success: function() {
                    ShowPic();
    ...
    if (confirm('ยืนยันการกำหนดเป็นภาพแรกของข่าว')) {
            $.ajax({
                data: {id: id},
                url: "<?= Yii::app()->params['MyBaseURL']; ?>Gallery/SetFirstPic&id=" + id,
                type: 'POST',
                dataType: 'json',
                success: function() {
                    ShowPic();
...
$this->widget('ext.EAjaxUpload.EAjaxUpload', array(
    'id' => 'uploadFile',
    'config' => array(
        'action' => Yii::app()->createUrl('Gallery/UploadPic', array("id" => $model->id)),
...
<script type="text/javascript">
    function ShowPic() {
        var url = "<?= Yii::app()->createUrl('Gallery/PicListByGallery', array('id' => $model->id)); ?>";
        $("#req_res").load(url);
        //alert(url);
    }
</script>
...
echo CHtml::ajaxLink(
        ...
        $showLink, array('Gallery/PicListByGallery', 'id' => $model->id), array(
